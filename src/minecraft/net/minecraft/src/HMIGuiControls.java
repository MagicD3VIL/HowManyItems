...
public class HMIGuiControls extends GuiScreen {
	...
	protected void keyTyped(char c, int i)
    {
        ...
        {
        	if(i == 1) i = 0;
        	if(binds[buttonId] == HMIConfig.toggleOverlay) {
        		for (int j = 0; j < mc.gameSettings.keyBindings.length; j++) {
        			if(mc.gameSettings.keyBindings[j] == HMIConfig.toggleOverlay) {
	...
	private static KeyBinding[] binds = { 
			HMIConfig.pushRecipe, 
			HMIConfig.pushUses, 
			HMIConfig.prevRecipe, 
			HMIConfig.allRecipes, 
			HMIConfig.clearSearchBox, 
			HMIConfig.focusSearchBox,
			HMIConfig.toggleOverlay
			};

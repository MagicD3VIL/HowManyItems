...
public class HMIGuiOptions extends GuiScreen {
    ...
    {
        int i = -1;
        controlList.add(buttonCheats = new GuiSmallButton(++i, (width / 2 - 155) + (i % 2) * 160, height / 6 + 24 * (i >> 1), "Mode: " + (HMIConfig.cheatsEnabled ? "Cheat Mode" : "Recipe Mode")));
	...
	protected void actionPerformed(GuiButton guibutton)
    {
        ...
        {
        	HMIConfig.cheatsEnabled = !HMIConfig.cheatsEnabled;
        	buttonCheats.displayString = "Mode: " + (HMIConfig.cheatsEnabled ? "Cheat Mode" : "Recipe Mode");
        }
        else if(guibutton == buttonIDs)
        {
        	HMIConfig.showItemIDs = !HMIConfig.showItemIDs;
        	buttonIDs.displayString = "Item IDs: " + (HMIConfig.showItemIDs ? "ON" : "OFF");
        }
        else if(guibutton == buttonCentredSearchBar)
        {
        	HMIConfig.centredSearchBar = !HMIConfig.centredSearchBar;
        	buttonCentredSearchBar.displayString = "Centred Search Bar: " + (HMIConfig.centredSearchBar ? "ON" : "OFF");
        }
        else if(guibutton == buttonFastSearch)
        {
        	HMIConfig.fastSearch = !HMIConfig.fastSearch;
        	buttonFastSearch.displayString = "Fast Search: " + (HMIConfig.fastSearch ? "ON" : "OFF");
        }
        ...
        {
        	HMIConfig.scrollInverted = !HMIConfig.scrollInverted;
        	buttonInvertedScroll.displayString = "Flip Scroll Direction: " + (HMIConfig.scrollInverted ? "ON" : "OFF");
        }

...
public class GuiOverlay extends GuiScreen {
	...
	public void drawScreen(int posX, int posY) {
		...
		if(shiftHeld && mod_HowManyItems.getTabs().size() > 0) {
			...
			buttonOptions.iconIndex = HMIConfig.cheatsEnabled ? 1 : 0;
			if(buttonTrash != null) {
		...
		if(HMIConfig.centredSearchBar) canvasHeight += BUTTON_HEIGHT;
	...
	public void mouseClicked(int posX, int posY, int eventButton) {
		...
		if(hoverItem != null && mc.thePlayer.inventory.getItemStack() == null) {
			if(mc.thePlayer.inventory.getItemStack() == null && HMIConfig.cheatsEnabled) {
				...
				if(eventButton == 0 || eventButton == 1) {
					...
					else if(HMIConfig.mpGiveCommand.length() > 0) {
						NumberFormat numberformat = NumberFormat.getIntegerInstance();
			            numberformat.setGroupingUsed(false);
			            MessageFormat messageformat = new MessageFormat(HMIConfig.mpGiveCommand);
		...
		if((mc.thePlayer.inventory.getItemStack() != null && !mc.theWorld.multiplayerWorld && (hoverItem != null || (posX > k + (w % 18)/2 && posY > screen.height - BUTTON_HEIGHT + (canvasHeight % 18) /2 - canvasHeight
				&& posX < screen.width - (w % 18)/2 && posY > BUTTON_HEIGHT + (canvasHeight % 18) /2  && posY < BUTTON_HEIGHT + canvasHeight))) && HMIConfig.cheatsEnabled) {
			if(eventButton == 0) {
		...
		else if(HMIConfig.cheatsEnabled && !mc.theWorld.multiplayerWorld && buttonTrash.mousePressed(mc, posX, posY) && mc.thePlayer.inventory.getItemStack() != null && eventButton == 1) {
	...
	protected void actionPerformed(GuiButton guibutton)
    {
		...
		if(guibutton == buttonNextPage) {
			...
			if(!mc.theWorld.multiplayerWorld) {
				...
				if(guibutton == buttonTimeDay) {
					mc.thePlayer.sendChatMessage(HMIConfig.mpTimeDayCommand);
				}
				else if(guibutton == buttonTimeNight) {
					mc.thePlayer.sendChatMessage(HMIConfig.mpTimeNightCommand);
				}
				else if(guibutton == buttonToggleRain) {
					try {
						WorldInfo worldInfo = (WorldInfo)worldInfoField.get(mc.theWorld);
						if(worldInfo.getRaining()) {
							mc.thePlayer.sendChatMessage(HMIConfig.mpRainOFFCommand);
						}
						else {
							mc.thePlayer.sendChatMessage(HMIConfig.mpRainONCommand);
						}
					} 
					catch (IllegalArgumentException e) { e.printStackTrace(); } 
			...
			if(!mc.theWorld.multiplayerWorld) {
				...
				mc.thePlayer.sendChatMessage(HMIConfig.mpHealCommand);
			}
		}
    ...
    {
		if(!searchBoxFocused() && HMIConfig.fastSearch && !mod_HowManyItems.keyHeldLastTick) {
			if(i != mc.gameSettings.keyBindInventory.keyCode && i != HMIConfig.allRecipes.keyCode && i != HMIConfig.toggleOverlay.keyCode
		...
		if(buttonNextPage != null) {
			for(Object obj : controlList) {
				...
				if(HMIConfig.overlayEnabled) {
					if(HMIConfig.cheatsEnabled) button.enabled2 = true;

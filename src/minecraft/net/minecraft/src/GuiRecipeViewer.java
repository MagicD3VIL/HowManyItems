...
public class GuiRecipeViewer extends GuiContainer
{
	...
	private void init() {
		if(HMIConfig.recipeViewerDraggableGui) {
	        xSize = HMIConfig.recipeViewerGuiWidth;
	        ySize = HMIConfig.recipeViewerGuiHeight;
		}
		else {
    ...
    protected void mouseClicked(int posX, int posY, int k)
    {
    	...
    	if(item != null && mc.thePlayer.inventory.getItemStack() == null) {
        	...
        	if (HMIConfig.recipeViewerDraggableGui && (posX - xSize + 10 > x) && (posX - xSize - 4 < x)
    ...
    protected void keyTyped(char c, int i)
    {
        ...
        if (i == Keyboard.KEY_ESCAPE || i == mc.gameSettings.keyBindInventory.keyCode) {
        	displayParent();
        	if(i == mc.gameSettings.keyBindInventory.keyCode) mc.displayGuiScreen(null);
        }
        else
        super.keyTyped(c, i);
    }
    ...
    {
    	if (HMIConfig.recipeViewerGuiWidth != xSize || HMIConfig.recipeViewerGuiHeight != ySize) {
    		HMIConfig.recipeViewerGuiWidth = xSize;
    		HMIConfig.recipeViewerGuiHeight = ySize;
